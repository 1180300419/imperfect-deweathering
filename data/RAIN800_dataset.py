...
class RAIN800Dataset(BaseDataset):
    def __init__(self, opt, split='train', dataset_name='GT-Rain'):
        super(RAIN800Dataset, self).__init__(opt, split, dataset_name)

        if self.root == '':
            rootlist = [
                '/hdd1/lxh/derain/dataset/Rain800/',
                '/data/wrh/lxh/derain/dataset/Rain800',
                '/home/user/files/data_set/Rain800'
            ]
    ...
    def _get_image_dir(self, root, split):
        ...
        if split == 'train':
            ...
            root_path = os.path.join(root, 'rain800_test')
        elif split == 'test':
            root_path = os.path.join(root, 'rain800_test')
        else:
            raise ValueError
        ...
        for img_path in os.listdir(root_path):
            ...
            rainy_dirs.append(os.path.join(root_path, img_path))
...
def imreader(arg):
    ...
    for _ in range(3):
        try:
            obj.imgs[i] = obj.imio.read(obj.rainy_dirs[i])
            failed = False
            break
        except:
            failed = True
