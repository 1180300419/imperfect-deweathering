...
class GTRAIN01Dataset(BaseDataset):
    def __init__(self, opt, split='train', dataset_name='GT-Rain'):
        super(GTRAIN01Dataset, self).__init__(opt, split, dataset_name)
    ...
    def _get_image_dir(self, root, split):
        ...
        if split == 'train':
            root_path = os.path.join(root, 'GT-RAIN_train')
        elif split == 'val':
            root_path = os.path.join(root, 'GT-RAIN_val')
        elif split == 'test':
            root_path = os.path.join(root, 'GT-RAIN_test')
        else:
            raise ValueError
        ...
        for scene in os.listdir(root_path):
            ...
            if scene == 'Gurutto_1-2':
                for img_path in os.listdir(scene_path):
                    if img_path[-9] == 'R':
        ...
        return names, scenes, rainy_dirs, clean_dirs
