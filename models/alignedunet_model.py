...
class ALIGNEDUNETModel(BaseModel):
    ...
    def __init__(self, opt):
        super(ALIGNEDUNETModel, self).__init__(opt)
        ...
        # self.netPWC = N.init_net(pwc, opt.init_type, opt.init_gain, opt.gpu_ids)
        # self.load_network_path(self.netPWC, '../ckpt/pwcnet-network-default.pth')
        if self.isTrain:
    ...
    def forward(self):
        if self.isTrain:
            ...
            # for i in range(3):
            self.derained_img = self.netUNET(self.derained_img)
...
class UNET(nn.Module):   
    def __init__(self, ngf=64, n_blocks=9, norm_layer_type='batch',
        ...
        self.pwcnet = PWCNet(load_pretrained=True, weights_path='../ckpt/pwcnet-network-default.pth')
