...
# 固定去雨网络，仅学习GCMModel的参数，查看GCMModel能否学习正确的颜色映射
class UNET2Model(BaseModel):
	...
	def __init__(self, opt):
		super(UNET2Model, self).__init__(opt)
		...
		if self.opt.test_internet:
			self.visual_names = ['rainy_img', 'derained_img', 'after_gcm_derained_img']
		else:
			self.visual_names = ['rainy_img', 'clean_img', 'derained_img', 'after_gcm_derained_img']
		...
		self.load_network_path(self.netUNET, '/hdd1/lxh/derain/code/checkpoints/unet-4resblocks-2e-4_rainaug/UNET_model_20.pth')
	...
	def forward(self):
		if self.isTrain:
			...
			self.after_gcm_derained_img = self.netGCM(derained_img, self.clean_img)
		else:
			self.derained_img = self.netUNET(self.rainy_img)
			derained_img = self.derained_img.clone().detach()
			self.after_gcm_derained_img = self.netGCM(derained_img, self.clean_img)
