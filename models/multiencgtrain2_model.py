...
class MULTIENCGTRAIN2Model(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--rain_variate_weight', type=float, default=0.0)

        parser.add_argument('--test_internet', type=bool, default=False)
        return parser

    def __init__(self, opt):
        super(MULTIENCGTRAIN2Model, self).__init__(opt)
        ...
        if self.isTrain:
            ...
            if self.opt.rain_variate_weight > 1e-6:
    ...
    def set_input(self, input, epoch):
        self.rainy_img = torch.cat(input['rainy_img'], dim=1).to(self.device)
    ...
    def forward(self):
        if self.isTrain:
            self.derained_img, self.derained_features = self.netUNET(self.rainy_img)
        else:
    ...
    def backward(self, epoch):
        ...
        if self.opt.rain_variate_weight > 0:
                self.loss_UNET_RAIN_VARIATE = self.criterionRainVarient(self.derained_features, self.clean_features, self.name).mean()
    ...
    def forward_x8(self, rainy_img, forward_function):
        aug_rainy = [rainy_img]
        
        aug_rainy.append(torch.flip(rainy_img, dims=[2, 3])) 
        aug_rainy.append(torch.flip(aug_rainy[0], dims=[3]))
        aug_rainy.append(torch.flip(aug_rainy[1], dims=[3]))
        
        aug_derained = []
        for tmp_rainy in aug_rainy:
            aug_derained.append(forward_function(tmp_rainy)[0])
        
        aug_derained[3] = torch.flip(aug_derained[3], dims=[3])
        aug_derained[2] = torch.flip(aug_derained[2], dims=[3])
        
        aug_derained[3] = torch.flip(aug_derained[3], dims=[2, 3])
        aug_derained[1] = torch.flip(aug_derained[1], dims=[2, 3])
        
        output_cat = torch.cat(aug_derained, dim=0)
...
class UNET(nn.Module):   
    ...
    def forward(self, rainy, clean=None):
        if clean is None:
            out_img, features = self.resnet(rainy)
            out_img = torch.clip(out_img, -1, 1)
            return out_img, features
        else:
            out_img, derained_features = self.resnet(rainy)
            out_img = torch.clip(out_img, -1, 1)
            clean = clean.repeat(1, 5, 1, 1)
            _, clean_features = self.resnet(clean)
            return out_img, derained_features, clean_features 
